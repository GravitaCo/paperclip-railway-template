...
RUN mkdir -p /paperclip/.local/bin \
  && if [ -x /root/.local/bin/claude ]; then cp -a /root/.local/bin/claude /usr/local/bin/claude 2>/dev/null || true; fi \
  && if [ -d /root/.claude ]; then cp -a /root/.claude /paperclip/.claude; fi
